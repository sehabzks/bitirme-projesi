...
public class PlayerController : MonoBehaviour
{
    ...
    IEnumerator Dash()
    {
        canDash = false;
        pState.dashing = true;
        pState.invincible = true; // <-- Dash başında zarar görmez yap
        anim.SetTrigger("Dashing");
        rb.gravityScale = 0;
        int _dir = pState.lookingRight ? 1 : -1;
        rb.linearVelocity = new Vector2(_dir * dashSpeed, 0); // "linearVelocity" yerine "velocity" olmalı
        ...
        pState.invincible = false; // <-- Dash bitince zarar alabilir olsun
    ...
    {
        yield return new WaitForSecondsRealtime(_delay); // RealTime çünkü timeScale = 0
        Time.timeScale = 1f;
        restoreTime = false;
    }
    public int Health
    ...
    void Heal()
    {
        if (Input.GetButton("Cast/Heal") && Health < maxHealth && Mana > 0 && Grounded() && !pState.dashing)
        {
            pState.healing = true;
